    ...
    if buy_status == False:
        if ticks["last_trade_time"] < next_candle_start_time:
            if buy_flag:
                    ...
                    stop_loss_price = ticks["last_price"]-2
                    buy_status = True
...
#
# dft=read_csv("/Users/mukul/PycharmProjects/AutomateTrading/trading_scripts/2023-02-19 01:11:30.684147_zero_pe_orders.csv")
# dft = dft[dft["status"]=="exit"]
# dft_p= dft[dft["profit"]>=0]
# dft_n= dft[dft["profit"]<0]

dftt = read_csv("/Users/mukul/PycharmProjects/AutomateTrading/configs/current_trading_data22.csv", on_bad_lines='skip')
profit = {}
x= set(dftt["instrument_token"])
for i in x:
    print(i)
    dft = dftt[dftt["instrument_token"] ==i]
    if len(dftt) ==0:
        continue
    ...
    ddd=read_csv(order_file_name)
    ddd=ddd.dropna()
    PROFIT = "profit"
    # print(ddd[PROFIT].value_counts())

    # print("profit ", ddd["profit"].sum())
    profit[i] = ddd["profit"].sum()
print("profit summry",profit)
