...
orders = {}
order_file_name = str(datetime.now()) + "_pe_orders.csv"
df = DataFrame(order_detail)
df.to_csv(order_file_name, index=False)

tick_data_file_name = "tick_data_pe.csv"
...
def on_ticks(ws, ticks):
    ...
    ticks = ticks[0]
    try:
        df = read_csv(tick_data_file_name)
    except:
        df = DataFrame(columns=list(ticks.keys()))
        df.to_csv(tick_data_file_name, index=False)
        df = read_csv(tick_data_file_name)
    df = concat([df, DataFrame(ticks)])
    df.to_csv(tick_data_file_name, index=False)
    if first == "stage_1":
    ...
    # trail stoploss
    print("buy_price: ", buying_price, "buy_status: ", buy_status, "order_exited:", order_exited(), "buy_flags:",
          buy_flag)
    print_candle("candle1", candle1)
    ...
    if buy_status == False:
        if ticks["last_trade_time"] < next_candle_start_time:
            if buy_flag:
                temp = min(candle1.open, candle1.close) + 2
                if temp >= ticks["last_price"]:
                    order_id = exc_order(
                        buy_at=ticks["last_price"],
                        sell_at=ticks["last_price"] + sell_margin,
                        stop_loss=ticks["last_price"] - candle1.low + 1
                    )
                    buying_price = ticks["last_price"]
                    selling_price = ticks["last_price"] + sell_margin
                    stop_loss_price = ticks["last_price"] - candle1.low + 1
                    buy_status = True
