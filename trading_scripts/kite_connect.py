...
orders = {}
order_file_name = str(datetime.now())+"_ce_orders.csv"
...
def on_ticks(ws, ticks):
    ...
    print()
    try:
        df = read_csv("tick_data_ce.csv")
    except:
        df = DataFrame(columns=list(ticks.keys()))
        df.to_csv("tick_data_ce.csv", index=False)
        df = read_csv("tick_data_ce.csv")
            ...
            if buy_flag:
                ...
                if temp >= ticks["last_price"]:
                    order_id = exc_order(
                        ...
                        stop_loss=ticks["last_price"] - min(ticks["last_price"]-candle1.low, max_stop_loss)
                    )
                    buying_price = ticks["last_price"]
                    selling_price = ticks["last_price"] + sell_margin
                    stop_loss_price = ticks["last_price"] - min(ticks["last_price"]-candle1.low, max_stop_loss)
                    buy_status = True
